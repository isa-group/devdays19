...
if (getLogsUrl() || getReporterUrl()) {
    var startInterval = () => {
        ...
        $scope.loopInterval = setInterval(function () {
            ...
            // Logs
            if (getLogsUrl()) {
                getUrl(getLogsUrl() + "/count", (err, count) => {
                    if (err) {
                        console.error(err);
                        $scope.intervalResults.logsStatus = "-";
                    } else {
                        console.info("Logs count", count);
                        $scope.intervalResults.logCount = count;
                        $scope.$apply();

                        getUrl(getLogsUrl() + "/ctrl/status", (err, data) => {
                            if (err) {
                                console.error(err);
                                $scope.intervalResults.logsStatus = "-";
                            } else {
                                console.info("Logs status", data.message);
                                let status = data.message === "Active" ? true : false;
                                $scope.intervalResults.logsStatus = status;
                            }
                            $scope.$apply();
                        });
                    }
                    // Update "logStatus" on template
                    $scope.$apply();
                });
            }
